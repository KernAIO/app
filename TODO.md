...
- [x] Every module carries an isolation test (2026-09-04: chat, quire, mail and billing joined
      ...
      **This list said ten and the database says twelve** (2026-09-05). `mod_core.memberships` and
      ...
      catalogue rather than the migration text, which is the only source that settles it. Ask
      `pg_class`/`pg_policy` on a real database; a `create policy` grep over the SQL cannot see a
      table nobody wrote one for.
      **Re-measured 2026-09-06 on a database created from nothing** — core plus all eight module
      folders applied with drizzle's migrator into a schema owned by a non-superuser role, which is
...
- [x] **The guard that should catch a missing policy cannot see one.** Only `chat`'s and `mail`'s
      `migrations.test.ts` asserted that no tenant table lacks a policy. The other five asserted
      `FORCE` on tables that already have one, so a table with no policy passed by never being
      looked at — which is exactly how twelve went uncounted while every module's test was green.
      Closed 2026-09-06: `tracker`, `quire`, `hr`, `billing` and `inventory` now ask the catalogue
      the other way round, and each names its exceptions in an `UNSECURED_BY_DESIGN` map carrying
      the reason and the file that isolates the table. The assertion also fails when the schema
      built no tenant table at all, so it cannot pass by finding nothing.
