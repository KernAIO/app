...
- Decision:
  1. Services live in separate repositories under `KernAIO` (`app`, `core`, `chat`, `mail`, `collab`); shared code in `kernel` (libs) and `modules` (first-party modules).
  2. Every backend service is an instance of the same runtime (`@kernaio/kernel`) hosting a configured list of modules. Which module runs where is configuration, not code.
